...
def getbinaryfile(
    ...
    if status != 200:
        ...
        error_messages = {
            408: "Request timeout - This usually means:\n"
                 "  • The firmware version does not exist for this model/region combination\n"
                 "  • The device IMEI/Serial is not recognized by Samsung servers\n"
                 "  • Try checking if the firmware version is correct with 'check' command",
            404: "Firmware not found - The specified firmware version does not exist",
            401: "Unauthorized - Invalid device credentials or authentication failed",
            500: "Server error - Samsung FUS server is experiencing issues",
        }
        error_msg = error_messages.get(status, f"Unknown error (status {status})")
        raise RuntimeError(f"DownloadBinaryInform failed with status {status}\n{error_msg}")
